...
function handle_incoming_request(req, res) {
    ...
    if (req.method === "GET" && queryObj.id) {
        ...
        qs.handleEventActions(req, res); // Handle actions like edit, delete, verify
    } else {
        fileServer.serve_static_file("html/event.html", res); // Serve the event page
    }
    break;
	...
	case "/make":
		if (req.method === "POST") {
			...
			qs.createEvent(req, res); // Create a new event
		} else {
			fileServer.serve_static_file("html/make.html", res); // Serve the create event page
		}
		break;
